...
INSERT INTO articulo VALUES (1234567891235,'Gomitas','Acidas',1,5);
...
SELECT A.codigo, A.nombre, A.descripcion, C.nombre, A.cantidad  FROM articulo A INNER JOIN categoria C ON A.categoria_id = C.id;
END ;;
DELIMITER ;
...
SELECT  A.codigo, A.nombre, A.descripcion, C.nombre, A.cantidad  FROM articulo A INNER JOIN categoria C ON A.categoria_id = C.id WHERE categoria_id = verificarCategoria;
...
SELECT A.codigo, A.nombre, A.descripcion, C.nombre, A.cantidad FROM articulo A INNER JOIN categoria C ON A.categoria_id = C.id WHERE codigo = p_codigo;
SET p_error = 0;
ELSE 
SET p_error = 1;
END IF;
